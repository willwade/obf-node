...
var Utils = {
  async get_url(url) {
    ...
    const extension = mime.extension(contentType) ? `.${mime.extension(contentType)}` : "";
    return {
  ...
  async identify_content(content, filename) {
    ...
    try {
      const parsedPlist = plist.parse(contentStr);
...
var Validator = class _Validator {
  ...
  async validate_obz_content_buffer(zipContent, filename) {
    ...
    await this.add_check("zip", "valid zip", async () => {
      try {
        zip = await JSZip2.loadAsync(zipContent);
        valid_zip = true;
      } catch (_e) {
        ...
        if (json.paths && json.paths.boards) {
          for (const [id, boardPath] of Object.entries(json.paths.boards)) {
            ...
            if (bFile) {
              ...
              const sub = await _Validator.validate_obf_content_static(bStr, boardPath, bData.length, { zipper: zip });
    ...
    if (Array.isArray(ext.sounds)) {
      for (let i = 0; i < ext.sounds.length; i++) {
        ...
        await this.add_check(`sounds[${i}]`, `sound at sounds[${i}]`, async () => {
          ...
          if (sound.duration !== void 0 && (typeof sound.duration !== "number" || sound.duration < 0))
    ...
    if (Array.isArray(ext.buttons)) {
      for (let i = 0; i < ext.buttons.length; i++) {
        ...
        await this.add_check(`buttons[${i}]`, `button at buttons[${i}]`, async () => {
          ...
          if (button.image_id === void 0 || button.image_id === null) {
    ...
    if (board.grid && board.grid.rows > 0 && board.grid.columns > 0) {
      ...
      for (let row = 0; row < board.grid.rows; row++) {
        for (let col = 0; col < board.grid.columns; col++) {
          ...
          const bgColor = button.background_color || button.style?.background_color ? utils_default.fix_color(button.background_color || button.style?.background_color, "hex") : "#ffffff";
          const borderColor = button.border_color || button.style?.border_color ? utils_default.fix_color(button.border_color || button.style?.border_color, "hex") : "#eeeeee";
          ...
          const fontColor = button.style?.font_color ? utils_default.fix_color(button.style.font_color, "hex") : "#000000";
            ...
            if (image) {
              try {
                ...
                if (image.data) {
                  ...
                  const response = await axios2.get(image.url, { responseType: "arraybuffer" });
...
var External = {
  async to_obf(hash, destPath, pathHash = null, toInclude = {}) {
    ...
    if (pathHash && pathHash.zip) {
      ...
      await fs4.writeJson(destPath, res, { spaces: 2 });
    }
    return destPath;
  },
  ...
  async to_external(filePath) {
    ...
    cellsArr.forEach((cell, idx) => {
      ...
      if (cell.commands && cell.commands.command) {
        const commands = Array.isArray(cell.commands.command) ? cell.commands.command : [cell.commands.command];
        commands.forEach((cmd) => {
          const type = cmd.id;
          if (type === "type") {
            button.vocalization = Array.isArray(cmd.parameter) ? cmd.parameter[0]._ : cmd.parameter._;
...
var TouchChat = {
  async to_external(filePath) {
    const tmpDir = fs7.mkdtempSync(path3.join(os.tmpdir(), "touchchat-"));
    let db = null;
    let imageDb = null;
    try {
      const zip = new AdmZip(filePath);
      zip.extractAllTo(tmpDir, true);
      const files = fs7.readdirSync(tmpDir);
      const vocabFile = files.find((f) => f.endsWith(".c4v"));
      if (!vocabFile) {
        throw new Error("No .c4v vocab DB found in TouchChat export");
      }
      const dbPath = path3.join(tmpDir, vocabFile);
      ...
      const variables = {};
      try {
        db.prepare("SELECT name, value FROM variables").all().forEach((v) => {
      ...
      try {
        db.prepare("SELECT * FROM button_styles").all().forEach((s) => buttonStyles.set(s.id, s));
        db.prepare("SELECT * FROM page_styles").all().forEach((s) => pageStyles.set(s.id, s));
      } catch (_e) {
      }
      ...
      const imageDbPath = path3.join(tmpDir, "Images.c4s");
      const imagesMap = /* @__PURE__ */ new Map();
      if (fs7.existsSync(imageDbPath)) {
      ...
      const buttonBoxCells = db.prepare(
        `
  ...
  async from_external(obf, outputPath) {
    const tmpDir = fs7.mkdtempSync(path3.join(os.tmpdir(), "touchchat-out-"));
    const vocabPath = path3.join(tmpDir, "vocab.c4v");
    const imagesPath = path3.join(tmpDir, "Images.c4s");
...
var Snap = {
  async to_external(filePath) {
    let db = null;
    try {
      db = new Database2(filePath, { readonly: true });
      ...
      const imagesMap = /* @__PURE__ */ new Map();
      try {
        const images = db.prepare('SELECT Id, Identifier, Data FROM PageSetData WHERE Identifier LIKE "IMG:%"').all();
        images.forEach((img) => {
      ...
      pages.forEach((pageRow) => {
        ...
        const board = {
          ...
          style: {
            background_color: pageRow.BackgroundColor ? `#${(pageRow.BackgroundColor & 16777215).toString(16).padStart(6, "0")}` : void 0
          },
          images: [],
          sounds: [],
      ...
      buttons.forEach((btnRow) => {
        ...
        if (board) {
          const button = {
            ...
            style: {
              background_color: btnRow.RefBackgroundColor || btnRow.BackgroundColor ? `#${((btnRow.RefBackgroundColor || btnRow.BackgroundColor) & 16777215).toString(16).padStart(6, "0")}` : void 0,
              border_color: btnRow.BorderColor ? `#${(btnRow.BorderColor & 16777215).toString(16).padStart(6, "0")}` : void 0,
              font_color: btnRow.RefForegroundColor || btnRow.LabelColor ? `#${((btnRow.RefForegroundColor || btnRow.LabelColor) & 16777215).toString(16).padStart(6, "0")}` : void 0
            }
          };
  ...
  async from_external(obf, outputPath) {
    ...
    const db = new Database2(outputPath, { readonly: false });
    try {
      db.exec(`
      ...
      boards.forEach((board) => {
        ...
        db.prepare(
          ...
          board.name,
          board.name,
          board.style?.background_color ? parseInt(board.style.background_color.replace("#", ""), 16) : null
        );
      });
      ...
      boards.forEach((board) => {
        ...
        board.buttons.forEach((btn) => {
          ...
          db.prepare(
            ...
            numericRefId,
            numericPageId,
            btn.style?.background_color ? parseInt(btn.style.background_color.replace("#", ""), 16) : null,
...
var Grid3 = {
  async to_external(filePath) {
    ...
    gridEntries.forEach((entry) => {
      try {
        ...
        if (grid) {
          ...
          const name = grid.Name || grid.name || path4.basename(path4.dirname(entry.entryName));
    ...
    gridEntries.forEach((entry) => {
      try {
        ...
        const name = String(grid.Name || grid.name || path4.basename(path4.dirname(entry.entryName)));
        ...
        try {
          ...
          if (fmEntry) {
            ...
            const fmEntries = fmData?.FileMap?.Entries?.Entry || fmData?.fileMap?.entries?.entry || [];
            (Array.isArray(fmEntries) ? fmEntries : [fmEntries]).forEach((ent) => {
              ...
              fileMapIndex.set(
                ...
                (Array.isArray(files) ? files : [files]).map(
                  (f) => (typeof f === "string" ? f : f["_"] || "").replace(/\\/g, "/")
                )
              );
            });
          }
        ...
        const resolveImage = (cellX, cellY, declaredName) => {
          ...
          for (const cand of candidates) {
            const found = zip.getEntry(cand.replace(/\//g, path4.sep)) || zip.getEntry(cand);
        ...
        if (cells) {
          ...
          cellArr.forEach((cell, idx) => {
            ...
            if (!content) return;
            let caption = content.CaptionAndImage?.Caption || content.captionAndImage?.caption || "";
            ...
            const declaredImage = content.CaptionAndImage?.Image || content.captionAndImage?.image || "";
            ...
            if (imgEntry) {
              ...
              if (!imgObj) {
                ...
                const ext = path4.extname(entryName).toLowerCase();
                imgObj = {
                  id: imgId,
                  data: imgEntry.getData().toString("base64"),
                  content_type: ext === ".png" ? "image/png" : ext === ".jpg" || ext === ".jpeg" ? "image/jpeg" : "image/png",
  ...
  async from_external(obf, outputPath) {
    const builder = new xml2js3.Builder();
...
export {
  ...
  grid3_default as Grid3,
  pdf_default as PdfBuilder,
  sfy_default as Sfy,
  sgrid_default as Sgrid,
  snap_default as Snap,
  touchchat_default as TouchChat,
