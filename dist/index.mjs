...
var External = {
  ...
  async from_obf(obfJsonOrPath, opts = {}) {
    ...
    if (typeof obfJsonOrPath === 'string') {
      const content = await fs4.readFile(obfJsonOrPath, 'utf8');
  ...
  async from_obz(obzPath) {
    const content = await fs4.readFile(obzPath);
    const zip = await JSZip3.loadAsync(content);
    const manifestFile = zip.file('manifest.json');
    if (!manifestFile) {
  ...
  async to_external(filePath) {
    ...
    gridEntries.forEach((entry) => {
      try {
        ...
        const name = String(
          grid.Name || grid.name || path4.basename(path4.dirname(entry.entryName))
        );
...
var Picto4me = {
  async to_external(zipPath) {
    ...
    const zip = await JSZip4.loadAsync(content);
