...
declare class Validator {
    ...
    static validate_content(content: Buffer | Uint8Array, filename: string, filesize: number): Promise<ValidationResult>;
    static validate_obf_content_static(content: string, filename: string, filesize: number, opts?: {
        zipper?: JSZip;
    }): Promise<ValidationResult>;
    static validate_obf_file(filePath: string, opts?: {
        zipper?: JSZip;
    }): Promise<ValidationResult>;
    static validate_obz_content_static(content: Buffer | Uint8Array, filename: string, filesize: number): Promise<ValidationResult>;
    static validate_obz_file(filePath: string): Promise<ValidationResult>;
    validate_obz_content_buffer(zipContent: Buffer | Uint8Array, filename: string): Promise<[ValidationCheck[], ValidationResult[]]>;
    validate_obf_content(content: string, filename: string, opts?: {
        zipper?: JSZip;
...
declare const External: {
    to_obf(hash: any, destPath: string | null, pathHash?: PathHash | null, toInclude?: {
        images?: boolean;
        sounds?: boolean;
    }): Promise<string | null>;
    to_obz(content: any, destPath: string, _opts?: {}): Promise<string>;
};
...
export { External, Grid3, type OBFButton, type OBFImage, type OBFPage, type OBFSound, type OBZPackage, PdfBuilder, Sfy, Sgrid, Snap, TouchChat, Utils, Validator };
