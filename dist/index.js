...
var Utils = {
  async get_url(url) {
    ...
    const extension = import_mime_types.default.extension(contentType) ? `.${import_mime_types.default.extension(contentType)}` : "";
    return {
            ...
            if (image) {
              try {
                ...
                if (image.data) {
                  ...
                  const response = await import_axios2.default.get(image.url, { responseType: "arraybuffer" });
...
var TouchChat = {
  async to_external(filePath) {
    const tmpDir = import_fs_extra5.default.mkdtempSync(import_path.default.join(import_os.default.tmpdir(), "touchchat-"));
    let db = null;
    let imageDb = null;
    try {
  ...
  async from_external(obf, outputPath) {
    const tmpDir = import_fs_extra5.default.mkdtempSync(import_path.default.join(import_os.default.tmpdir(), "touchchat-out-"));
  ...
  async to_external(filePath) {
    ...
    gridEntries.forEach((entry) => {
      try {
        ...
        if (grid) {
          ...
          const name = grid.Name || grid.name || import_path2.default.basename(import_path2.default.dirname(entry.entryName));
    ...
    gridEntries.forEach((entry) => {
      try {
        ...
        const name = String(grid.Name || grid.name || import_path2.default.basename(import_path2.default.dirname(entry.entryName)));
        ...
        const resolveImage = (cellX, cellY, declaredName) => {
          ...
          for (const cand of candidates) {
            const found = zip.getEntry(cand.replace(/\//g, import_path2.default.sep)) || zip.getEntry(cand);
