...
class Validator {
      ...
      if (json) {
        await this.add_check("manifest_format", "manifest.json format version", async () => {
          ...
          if (version > OBF_FORMAT_CURRENT_VERSION) {
            self.err(`format version (${version}) is invalid, current version is ${OBF_FORMAT_CURRENT_VERSION}`);
          } else if (version < OBF_FORMAT_CURRENT_VERSION) {
            self.warn(`format version (${version}) is old, consider updating to ${OBF_FORMAT_CURRENT_VERSION}`);
          }
        });
        ...
        await this.add_check("manifest_extras", "manifest.json extra attributes", async () => {
            ...
            Object.keys(json).forEach(key => {
                if (!attrs.includes(key) && !key.startsWith('ext_')) {
                    self.warn(`${key} attribute is not defined in the spec, should be prefixed with ext_yourapp_`);
                }
            });
            ...
            Object.keys(json.paths).forEach(key => {
                if (!pathAttrs.includes(key) && !key.startsWith('ext_')) {
                    self.warn(`paths.${key} attribute is not defined in the spec, should be prefixed with ext_yourapp_`);
                }
            });
        });
                ...
                await this.add_check(`manifest_boards[${id}]`, `manifest.json path.boards.${id}`, async () => {
                    ...
                    try {
                        ...
                        if (!boardJson || boardJson.id !== id) {
                            ...
                            self.err(`board at path (${path}) defined in manifest with id "${id}" but actually has id "${boardId}"`);
                        }
                    } catch (e) {
  ...
  async validate_obf_content(content, filename, opts = {}) {
    ...
    await this.add_check("extras", "extra attributes", async () => {
        const attrs = ['format', 'id', 'locale', 'url', 'data_url', 'name', 'description_html', 'default_layout', 'buttons', 'images', 'sounds', 'grid', 'license'];
    ...
    await this.add_check("grid", "grid attribute", async () => {
        ...
        if (typeof ext.grid.rows !== 'number' || ext.grid.rows < 1) self.err("grid.rows must be a positive number");
        if (typeof ext.grid.columns !== 'number' || ext.grid.columns < 1) self.err("grid.columns must be a positive number");
        if (!ext.grid.order || !Array.isArray(ext.grid.order)) self.err("grid.order must be an array of arrays");
        if (ext.grid.order.length !== ext.grid.rows) self.err(`grid.order length (${ext.grid.order.length}) must match grid.rows (${ext.grid.rows})`);
        if (!ext.grid.order.every(r => Array.isArray(r) && r.length === ext.grid.columns)) {
            self.err(`grid.order must contain ${ext.grid.rows} arrays each of size ${ext.grid.columns}`);
        }
    });
    ...
    await this.add_check("grid_ids", "button IDs in grid.order attribute", async () => {
                    ...
                    row.forEach(id => {
                        if (id !== null && id !== undefined) {
                            ...
                            if (!buttonIds.includes(id)) {
                                self.err(`grid.order references button with id ${id} but no button with that id found in buttons attribute`);
                            }
                        }
                    });
                }
        ...
        if (unusedIds.length > 0) {
            self.warn(`not all defined buttons were included in the grid order (${unusedIds.join(",")})`);
        }
    });
    ...
    if (Array.isArray(ext.images)) {
        for (let i = 0; i < ext.images.length; i++) {
            ...
            await this.add_check(`image[${i}]`, `image at images[${i}]`, async () => {
                ...
                if (!image.width || typeof image.width !== 'number' || image.width < 1) self.err("image.width must be a valid positive number");
                if (!image.height || typeof image.height !== 'number' || image.height < 1) self.err("image.height must be a valid positive number");
                if (!image.content_type || !image.content_type.match(/^image\/.+$/)) self.err("image.content_type must be a valid image mime type");
                if (!image.url && !image.data && !image.symbol && !image.path) self.err("image must have data, url, path or symbol attribute defined");
                
                const imageAttrs = ['id', 'width', 'height', 'content_type', 'data', 'url', 'symbol', 'path', 'data_url', 'license'];
                Object.keys(image).forEach(key => {
                    if (!imageAttrs.includes(key) && !key.startsWith('ext_')) {
                        self.warn(`image.${key} attribute is not defined in the spec, should be prefixed with ext_yourapp_`);
                    }
                });
            });
        }
    ...
    if (Array.isArray(ext.sounds)) {
        for (let i = 0; i < ext.sounds.length; i++) {
            ...
            await this.add_check(`sounds[${i}]`, `sound at sounds[${i}]`, async () => {
                ...
                if (sound.duration !== undefined && (typeof sound.duration !== 'number' || sound.duration < 0)) self.err("sound.duration must be a valid positive number");
                if (!sound.content_type || !sound.content_type.match(/^audio\/.+$/)) self.err("sound.content_type must be a valid audio mime type");
                if (!sound.url && !sound.data && !sound.symbol && !sound.path) self.err("sound must have data, url, path or symbol attribute defined");
            });
        }
    }
    ...
    if (Array.isArray(ext.buttons)) {
        for (let i = 0; i < ext.buttons.length; i++) {
            ...
            await this.add_check(`buttons[${i}]`, `button at buttons[${i}]`, async () => {
                ...
                if (!button.label && !button.vocalization) self.err("button.label or button.vocalization is required");
                
                ['background_color', 'border_color'].forEach(color => {
                    if (button[color]) {
                        if (!button[color].match(/^\s*rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*(,\s*[01]\.?\d*)?\)\s*/)) {
                            self.err(`button.${color} must be a valid rgb or rgba value if defined ("${button[color]}" is invalid)`);
                        }
                    }
                });
                ...
                if (button.load_board && button.load_board.path) {
                    if (!opts.zipper) {
                        ...
                        self.err(`button.load_board.path references ${button.load_board.path} which isn't found in the zipped file`);
                    }
                }
            });
        }
